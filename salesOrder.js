...
class salesOrder {
    ...
    async checkAccessValidity(companyUUID, editorEmail, action){
        try {
            ...
            if (findEmployee == -1) {
                ...
                if (!await this.uamObject.verifyAccess(employees[findEmployee].uam, "salesOrder", action)) {
    ...
    async getSalesOrderList(token, companyUUID, editorEmail, page, size, sort = null) {
        try {
            // Verify the token
    ...
    async getSalesOrderDetail(token, companyUUID, editorEmail, itemSKU, itemUUID) {
        try {
            // Verify the token
            await this.userObject.verifyToken(token);
    ...
    async createNewSalesOrder(token, payload) {
    ...
    async editExistingSalesOrder(token, payload) {
        return new Promise(async (resolve, reject) => {
            try {
                // Verify the token
                await this.userObject.verifyToken(token);
    ...
    async editSalesOrderQTY(payload) {
        return new Promise(async (resolve, reject) => {
            try {
                // Check if payload is an object
                if (typeof payload !== 'object') {
    ...
    async deleteSalesOrder(token, payload) {
        return new Promise(async (resolve, reject) => {
            try {
                // Verify the token
                await this.userObject.verifyToken(token);
